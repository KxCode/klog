...
def scrubFile(filePath):
  ...
  with open(filePath, "rb") as f:
    ...
    while bytes != "":
      ...
      if cut(data[4]) != usernameToRemove or cut(data[5]) != hostAddressToRemove:
        newUtmp += bytes
      bytes = f.read(UTMP_STRUCT_SIZE)
    f.close()
  return newUtmp
...
def scrubLastlogFile(filePath, userName):
  ...
  idCount = 0
  newLastlog = ''
  # each struct line in lastlog means a user
  with open(filePath, "rb") as f:
    ...
    while bytes != "":
      ...
      if (idCount != uid):
        ...
        #print 'found target uid' + str(idCount)
        #fill in null bytes for this uid line
        data = (0,0,'','')
        newLastlog += struct.pack("hh32s256s",*data)
      idCount += 1
      bytes = f.read(LASTLOG_STRUCT_SIZE)
  return newLastlog
